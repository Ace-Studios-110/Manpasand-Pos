...
class TaxService {
    async createTax(data) {
        ...
        const tax = await db_1.prisma.tax.create({
            data: {
                ...data,
                code: newCode
            },
    ...
    async getTaxById(id) {
        const tax = await db_1.prisma.tax.findUnique({
            where: { id },
            include: {
                products: {
