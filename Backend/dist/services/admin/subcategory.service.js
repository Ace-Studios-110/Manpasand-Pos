...
class SubcategoryService {
    async createSubcategory(data) {
        const existingSubcategory = await db_1.prisma.subcategory.findFirst({
            where: {
                name: data.name,
                is_active: true,
            },
        ...
        const lastSubcategory = await db_1.prisma.subcategory.findFirst({
    ...
    async getSubcategoryById(id) {
        const subcategory = await db_1.prisma.subcategory.findUnique({
            where: { id },
            include: {
                products: {
    ...
    async listSubcategories({ page = 1, limit = 10, search, is_active = true, display_on_pos = true, }) {
        ...
        const [subcategories, total] = await Promise.all([
            db_1.prisma.subcategory.findMany({
                where,
                skip: (page - 1) * limit,
                take: limit,
                orderBy: { created_at: 'desc' },
